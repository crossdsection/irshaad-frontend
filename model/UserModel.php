<?php
		...
		public function __construct($flag='', $data='', $file='')
		{
			...
			if($flag == '')
			{
				if($data != '')
				{
					...
					if((isset($data['country'])) && ($data['country'] != ''))   // for super-admin
					{
					...
					if((isset($data['state'])) && ($data['state'] != ''))     // for super-admin
					{
					...
					if((isset($data['city'])) && ($data['city'] != ''))       // for super-admin
					{
						$this->city = $data['city'];
					}
					else
					...
					if((isset($data['type_flag'])) && ($data['type_flag'] != ''))       // for super-admin
					{
					...
					if((isset($data['department_level_flag'])) && ($data['department_level_flag'] != '')) // for super-admin
					{
					...
					if((isset($data['authority_depart_data'])) && ($data['authority_depart_data'] != ''))   // for superAdmin
					{
					...
					if((isset($data['company_name'])) && ($data['company_name'] != ''))   // for superAdmin
					{
					...
					if((isset($data['rwa_name'])) && ($data['rwa_name'] != ''))   // for superAdmin
					{
	...
	public function subAdminSignup()
	{
	 ...
	 public function verifySubAdminEmail()
	 {
		...
		{
			 $updata_subAdmin = $this->update("wv_authority",array('email_verify'=>1),"email = '$this->subAdminEmail'");
		...
		public function verifyEmail()
		{
			...
			if(count($emailData))
			{
			    ...
			    {
					$result = $this->update($this->tableName,array('email_verified'=>1),"email='$this->verify_email'");
					if($result != false)
					{
						die(json_encode(array(
						 'status' => 1,
		...
		public function gmailLogin()
		{
		  ...
		  if(count($check)>0)
		  {
			   ...
			   $updateOnlineStatus = $this->update($this->tableName, array('active' => 1, 'email_verified' => 1), "id= '{$check[0]['id']}'"); 
		...
		public function changeProfileImage()
		{
			...
			if(count($getSubadminData)>0)
			{
				...
				if($getSubadminData['department_id'] != '0')
				{
					...
					$updateDepartmentHeadImage = $this->update('wv_ministry', array('ministry_head_profilepic' => $image),"wv_ministry.id = '{$getSubadminData['department_id']}'");
				}
				
			}
			
			$updateSubadminProfile = $this->update("wv_authority", array('profile_picture' => $image), "wv_authority.id = '{$this->id}'");
