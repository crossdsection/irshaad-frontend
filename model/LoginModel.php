<?php
	...
	class LoginModel extends Database
	{
		...
		public function subAdminLogin($data)
		{
			
			$subAdminLoginData = $this->select("*","wv_authority","email = '{$data['email']}' and autority_role = '{$data['auth_role']}'");
			
			
			if(count($subAdminLoginData)>0)
			{
				$subAdminLoginData = $subAdminLoginData[0];
				
				if($subAdminLoginData['password'] == md5($data['password']))
				{
					$updateSubAdminOnlineStatus = $this->update('wv_authority', array('active' => 1), "id = '{$subAdminLoginData['id']}'");
					 
					
					$session = new Session(array(
						 'id' => $subAdminLoginData['id'],
						 'firstName' => $subAdminLoginData['first_name'],
						 'lastName' => $subAdminLoginData['last_name'],
						 'gender' => $subAdminLoginData['gender'],
						 'email' => $subAdminLoginData['email'],
						 'phone' => $subAdminLoginData['phone'],
						 'address' => $subAdminLoginData['country_id']."`".$subAdminLoginData['state_id']."`".$subAdminLoginData['city_id'],
						 'profile' => $subAdminLoginData['profile_picture']
					));
					
					
					$_SESSION['subAdmin'] = serialize($session);
	    ...
	    public static function logout() 
		{
			if(isset($_SESSION)) 
			{
				...
				else
				{
					...
					$db = new Database();
					$db->update(self::$table_Name1, array('active' => 0), "id = '{$subadmin->id}'");
				}
			}
