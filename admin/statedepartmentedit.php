<?php 
	...
	{
		$getStateData = $db->select("wv_ministry.*, wv_countries.name as country_name, wv_states.name as state_name","wv_ministry left join wv_countries on wv_ministry.country_id = wv_countries.id left join wv_states on wv_ministry.state_id = wv_states.id","wv_ministry.ministry_status = 2 and wv_ministry.id = '{$_GET['edit']}'");
...
?>
    <body>
        <div id="wrapper">
            ...
            <div id="page-wrapper">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">Edit State Ministry</h1>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                <!-- /.row -->
 ...
 <!-- DataTables JavaScript -->
		...
		<script>
		   $(document).on("submit", ".state-ministry", function(event){
			   ...
			   if(state_name == '00')
			   {
			    ...
			    var ministry_name = $(this).find('input[name="state_ministry_name"]').val();
